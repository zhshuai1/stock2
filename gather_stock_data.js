...
function getPersister(){
    ...
    var insert=function(entry){
        ...
        connection.query('INSERT INTO Stocks2 (code,open,close,high,low,year,month,day,date,delta,volume) values (?,?,?,?,?,?,?,?,?,?,?);',entry, 
    ...
    var update=function(entry){
        ...
        connection.query('Update Stocks2 set code=?,open=?,close=?,high=?,low=?,year=?,month=?,day=?,date=?,delta=?,volume=? where code=? and year=? and month=? and day=?;',entry, 
        function (err, results, fields) {  
            if (err){  
                throw err;  
            } 
    ...
    var persister={
        persist:function(code,entry){
            ...
            insert(params);
            return;
            connection.query("SELECT * FROM Stocks2 where code=? and year=? and month=? and day=?;",[code,date.getYear(),date.getMonth(),date.getDate()], 
            function (err, results, fields) {  
                if (err){  
                    throw err;  
                } 
        ...
        checkOnceAndPersist:function(code,histories,num){
            var p=this;
            connection.query("SELECT * FROM Stocks2 where code=?;",[code], 
            function (err, results, fields) {  
                if (err){  
                    throw err;  
                } 
	...
	var gatherer={
	    getHistories:function(code,handle){
	        var options={
		        ...
		        port: 80,
		        path: "/realstock/company/sz"+code+"/hisdata/klc_kl.js",
		        method: "GET"
            };
...
function gatherData(){
    ...
    //var stocks=stocks2.slice(1350,1500);
    var stocks=['000001'];
