        ...
        var handler = function (resolve, reject) {
            connection.query('REPLACE INTO stock VALUES(?,?,?,?,?,?,?,?,?);',
                ...
                function (error, results, fields) {
                    ...
                    log.info("Successfully persist entry for ", entry.code, " on ", entry.date);
        ...
        getHistories: function (type, code) {
            ...
            var p = function (resolve, reject) {
                var req = Http.request(options, function (res) {
                    ...
                    reject("Error occured when retrieving data from sina." + e);
                });
                req.end();
            }
...
var pool = function (type, stocks, days, poolSize) {
    ...
    var submitOne = function () {
        ...
        if (!stock) return;
        log.info("The next stock to process is: " + stock);
        processOneStock(type, stock, days).then(success, fail);
    };
    var success = function (response) {
        ++index;
        log.info("Successfully processed " + response + ": " + index + "/" + total + " stocks.");
        submitOne();
    };
    var fail = function (errInfo) {
        var failedStock = errInfo[0];
        fails[failedStock] = (fails[failedStock] || 0) + 1;
        if (fails[failedStock] >= 5) {
            log.error("The stock has failed for more than 5 times: " + type + failedStock);
            stocks.unshift(errInfo[0]);
        }
        submitOne();
    };
    return {start: start};
    ...
    var p = function (resolve, reject) {
        ...
        g.getHistories(type, code).then(function (histories) {
            ...
            Promise.all(promises).then(function () {
                resolve(fullCode);
                log.info("Successfully persist for stock " + fullCode);
                p.destroy();
            }, function () {
                reject(fullCode);
