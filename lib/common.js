...
var prepareData = function (data, number, type, code, authorities, date2Detail) {
    ...
    for (var i = start; i < size; ++i) {
        ...
        var thisValue = authorities[0].data[formKey(data[i].date)]
        var lastValue = authorities[0].data[formKey(data[i - 1].date)]
...
var processOneStock = function (type, code, days) {
    ...
    var promise = function (resolve, reject) {
        ...
        Promise.all([g.getHistories(type, code), g.getForwardAuthority(type, code)]).then(function (result) {
                ...
                var authoritiesExp = result[1].match(/=(.*)/)[1];
                console.log(authoritiesExp)
                eval('var authorities=' + authoritiesExp)
                // dedup
                var monthSet = new Set()
                for (var k in authorities[0].data) {
                    var yearMonthDay = k.split('_')
                    var year = yearMonthDay[1]
                    var month = yearMonthDay[2]
                    var day = yearMonthDay[3]
                    if (new Date().getTime() - new Date(year, +month - 1, day).getTime() <= days * 86400 * 1000) {
                ...
                Promise.all(minuteDataPromises).then(function (minuteData) {
                    //console.log(JSON.stringify(minuteData))
                    var date2Detail = {}
